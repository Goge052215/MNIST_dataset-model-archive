...
from torch.optim.adam import Adam  # Update this line
...
def train(model, device, train_loader, optimizer, criterion, epoch):
    model.train()
    train_loader_tqdm = tqdm(train_loader, desc=f"Epoch {epoch}", leave=False)  # tqdm for progress
    for batch_idx, (data, target) in enumerate(train_loader_tqdm):
        ...
        target_onehot = torch.eye(10, device=device)[target]  # Move torch.eye to the same device
...
def test(model, device, test_loader, criterion):
    ...
    test_loss = 0
    correct = 0
    test_loader_tqdm = tqdm(test_loader, desc="Testing", leave=False)  # tqdm for progress
    with torch.no_grad():
...
optimizer = Adam(model.parameters(), lr=0.001)  # Update this line
